...
void s21::ConsoleMenu::test() {
  Tester test;
  bool result(test.testLoad() && test.testExport() && test.testDepth() &&
              test.testBreadth() && test.testShortesPath() &&
              test.testShortesPathAll() && test.testListSpaningTree() &&
              test.testSalesman());
  std::cout << "All tests " << (result ? "passed" : "failed") << std::endl;
}
